...
### 11 2021_《TDN: Temporal Difference Networks for Efficient Action Recognition》
...
本文纯粹是一种提升行为识性能的模型的研究。并不涉及特定难题。它提出了一个新的架构，**专注于计算多尺度时序信息**

核心：我们的**TDN的核心**是通过明确利用**时序差分算子**来设计有效的**时序模块**（TDM），并系统地评估其对**短期和长期**运动建模的影响。

在整个视频中充分捕捉时序信息的方法：我们的TDN是采用**两级差分建模范式two-level difference modeling paradigm**建立的。**具体的实现过程：**

- 对于局部运动建模，使用**连续帧上的时间差**来提供具有更精细运动模式的**二维CNN**；
- 而对于全局运动建模，使用**跨段的时间差**来捕获运动特征激励的**长程结构**。

TDN提供了一个**简单而有原则**的时态建模框架，可以用现有的CNN以**较小的额外计算成本**进行**实例化**。

数据集：something V1 V2 Kinectics-400；

#### 引言

时序建模对捕获motion动态特征信息很重要，同时它通常由两种机制实现的。
...
- 普遍做法：使用双流网络。
    ...
    这种方式被证明是高效的，但是在光流数据计算上，需要大量的计算消耗。

- 3D卷积或者时序卷积：从RGB帧**隐式学习**运动motion特征。
    ...
    因此，设计一种高效的**高运动建模能力**的**时序模块**，在视频识别领域依然是一个挑战性的问题。

- 本文的目标：

    通过一种时序差分模块，呈现一个全新的时序建模机制。

    - 时序差分与光流高度相关，同时【38，43】的研究表明了使用RGB差分信息作为一种近似的motion运动表示的方式已经表明其有效性。
    - 但是，【11，38，43】这些文献也仅仅时将RGB差分信息作为另一种视频模态，同时为其设计一个融合了RGB网络的不同的网络。然而，我们的目标时呈现一个同意的框架，以捕捉到外观和运动信息，**通过将时序差分的思想泛化到一个具有原则的和搞笑的时序模块。**

- 作者的其他思想：

    认为无论长期还是短期的时序信息都是非常重要的，在某种意义上，这些信息都能够捕获到行为实例的区分性的和补充性的信息。

    因此，在所提出的时序建模机制中，作者呈现了一个两级时序建模框架，**基于整体和稀疏采样策略**，被称为TDN。

    具体地解释：

    - 我们考虑两种有效形式的**时序差分模块**在不同尺度上的运动建模。
    - 局部的motion建模，轻量级、低分辨率的差分模块，通过**横向连接**为**单个RGB**提供**运动模式**。
    - 长期motion建模，多尺度和双向差分模块以捕获跨段变化信息。用于motion运动激励。

- 基于CNN网络：

    - ResNets，两个数据集；SS是运动相关数据集，Kinetics是场景相关数据集。
    - 做了消融实验，研究TDM模块的设计。

- 贡献的两个方面：

    - 将RGB差分的想法泛化为一个时序差分模块，作为3D卷积的一种高效的detailed module细致的模块。
    - 呈现一个视频级别的运动建模框架，专注于长期和短期的时序结构；
    - 实验证明不错。同时消融实验结果喜人。
...
- 双流CNN网络和其变体。
    ...
    后期将两种模态的apperance 和 motion信息融合。
...
- 三维卷积网络以及变体：
    ...
    - 设计能力更强悍的时序模块，嵌入到二维卷积网络中：TSM\TIN]TEINet\TEA等。
    - 借助双流框架设计一个多分支架构，以捕捉到外观信息和运动信息或则会上下文信息，（借助一个特别设计的时序模块或者两个RGB输入，采样自不同的FPS，包括非局域网络，STM和SlowFast。）
...
- TSN，证明了一个有效的长期建模框架，同时使用了二维卷积，并取得了不错的结果

    但是，基于二维卷积的TSN网络仅仅在最后阶段执行时序融合，没能捕获到更加细微的时序结构；

- STNET，局域和全局模块以对时序信息进行层次性质的建模；

- V4D，思维卷积算子以融合不同阶段的长期信息。

**时序差分表示**

常用语motion运动特征的提取，比如RGB差分和特征的差分。

- RGB差分，表明光流模态的替代方案是有效的；

    **但是过去的研究仅仅将RGB视为另一种视频模态，同时为其单独训练一个网络以融合RGB流**

- 特征差分，STM TEA等模型；

但是，以上两种方案的特点是：仅仅使用一个简单的差分算子，用于单级motion特征的提取，获取到比三维卷积较少的研究注意信息。

由此，TDN。

![image-20210927111935286](/home/cold/PaperReadFastly/PaperRead/论文阅读列表/2021年度/B驾驶行为与图卷积网络/6月以后/VideoUnderstanding.assets/image-20210927111935286.png)
...
新的研究少样本行为识别的方法：在查询和支持集中视频之间搜寻到 **时序对应的帧元祖**

方法的新：通过交叉转换注意力机制，构建**类原型**，以在支持视频集中找到相应的子序列。**而不是使用类平均或者单一最优匹配。**

**视频表示**由不同帧数的**有序**元组形成，允许比较**不同速度和时间偏移**下的动作子序列。

新的方法的优越表现：因为对时序关系的建模，得到了超越前者12%的提升；

消融实验：研究匹配到多个支持集中的视频的重要性，以及学习更有序的关系性的交叉转换。



#### 引言

少样本旨在通过少量标注的样本以学习到新的类别。

因为不同的行为长度和时序依赖，使得少样本视频识别任务也面临很多的挑战。

**受到基于部分的少样本图片分类的启发**，

- 我们认为，在构建类原型的时候，如果将所查询的视频的子序列和所有支持集视频的子序列进行比较的话，对于少样本的regime组织方式，会有好处。
- 通过在**不同的时间位置和位移处匹配**子序列，这可以**更好**地积累证据。



一个特定的查询类原型通过使用一个注意力机制以匹配每个查询子序列
