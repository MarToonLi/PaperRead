...
def get_norm(name, c_out):
    if name == 'batch':
        norm = nn.BatchNorm2d(c_out)  # 面向一个batchsize个样本的CHW的元素；
    elif name == 'instance':
        norm = nn.InstanceNorm2d(c_out)  # 面向一个样本的一个通道的HW的元素；
    else:
        norm = None
    return norm
...
# 获取激活函数层
def get_act(name, inplace=False):
    if name == 'relu':
        activation = nn.ReLU(inplace)
    elif name == 'elu':
        activation = nn.ELU(inplace)
    elif name == 'leaky_relu':
        activation = nn.LeakyReLU(negative_slope=0.2, inplace=inplace)
    elif name == 'tanh':
        activation = nn.Tanh(inplace)
    elif name == 'sigmoid':
        activation = nn.Sigmoid(inplace)
    else:
        activation = None
    return activation
