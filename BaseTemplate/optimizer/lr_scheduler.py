...
# -*- coding:utf-8 -*-
# 代码 copy from :
# https://github.com/Megvii-BaseDetection/YOLOX/blob/c9fe0aae2db90adccc90f7e5a16f044bf110c816/yolox/utils/lr_scheduler.py#L9
...
class LRScheduler:
    def __init__(self, name, lr, iters_per_epoch, total_epochs, **kwargs):
        ...
        # 将kwargs这个字典添加到该类的属性dict中 ---- 一种动态定义类中属性的方式.
        self.__dict__.update(kwargs)
        # 初始化优化器的学习率. 是 update_lr 方法的基础.
...
def warm_cos_lr(lr, total_iters, warmup_total_iters, warmup_lr_start, iters):
    ...
    if iters <= warmup_total_iters:
        ...
        lr *= 0.5 * (1.0 +
                     math.cos(math.pi *
                              (iters - warmup_total_iters) / (total_iters - warmup_total_iters)
                              )
                     )
    return lr
...
def yolox_warm_cos_lr(
        lr, iters, total_iters, warmup_total_iters, warmup_lr_start, # 与warm_cos_lr一致;
        min_lr_ratio, no_aug_iter # 新增的内容
):
    min_lr = lr * min_lr_ratio  # self.min_lr_ratio = 0.05 原文的设置
    if iters <= warmup_total_iters:
        lr = (lr - warmup_lr_start) * pow(iters / float(warmup_total_iters), 2) + warmup_lr_start
    elif iters >= total_iters - no_aug_iter:
        # 如果迭代次数达到最后几个迭代的时候,就使用某个lr.
        lr = min_lr
    else:
